      ...
      >
        <div className="max-w-2xl w-full relative z-10">
          ...
          <div className="flex flex-wrap items-center gap-2 mb-8">
            <span className="text-xs text-gray-300 flex items-center gap-1 mr-2 font-medium">
              <Filter size={12} className="text-[#C4A0F5]" />
              <span>Filter:</span>
            </span>
            {categories.map((cat) => (
              <button
                ...
                className={`px-3.5 py-1.5 rounded-lg text-xs font-mono tracking-wider transition-all duration-300 cursor-pointer ${
                  selectedCategory === cat
                    ? "bg-[#6D28D9] text-white font-bold shadow-md shadow-purple-900/50 border border-[#C4A0F5]/40"
                    : "bg-[#150F1D] text-gray-300 border border-[#2B1B48] hover:text-white hover:border-[#C4A0F5]/40"
                }`}
              >
                {cat}
              </button>
          ...
          <div className="space-y-5">
            {filteredProjects.map((project) => (
              <div
                ...
                className="glass-panel p-6 rounded-2xl group cursor-pointer relative overflow-hidden transition-all duration-300 border border-[#2D1B4D] hover:border-[#8B5CF6]"
              >
                ...
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                  <div className="space-y-2 max-w-lg">
                    ...
                    <div className="flex flex-wrap gap-2 pt-2">
                      {project.tags.map((tag, tIdx) => (
                        <span
                          key={tIdx}
                          className="text-[11px] font-mono px-2.5 py-0.5 rounded bg-[#191124] text-gray-200 border border-[#2D1C48]"
                        >
                          {tag}
                        </span>
                      ))}
                  ...
                  {/* Actions & Buttons */}
                  <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-start gap-3 shrink-0 pt-3 sm:pt-0">
                    <div className="flex items-center gap-2">
                      <a
                        ...
                        className="p-2 rounded-xl bg-[#1D1429] border border-[#351D57] text-gray-200 hover:text-white hover:border-[#C4A0F5] transition-all cursor-pointer"
