...
export default function TerminalWidget() {
  ...
  return (
    <div className="glass-panel rounded-xl border border-[#3A1F60] overflow-hidden my-8">
      {/* Terminal Title Bar */}
      <div className="bg-[#160E1E] px-4 py-3 border-b border-[#2C1744] flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="ml-2 text-xs font-mono text-[#DDD6FE] font-medium flex items-center gap-1.5">
            <TerminalIcon size={14} className="text-[#A78BFA]" />
      ...
      <div className="bg-[#0F0816] px-4 py-2.5 border-b border-[#231238] flex flex-wrap items-center gap-2">
        <span className="text-[11px] font-mono text-slate-400 font-medium">Quick CLI:</span>
        {["help", "whoami", "skills", "projects", "contact", "time", "status"].map((cmd) => (
          <button
            key={cmd}
            onClick={() => handleQuickCmd(cmd)}
            className="text-[11px] font-mono px-2.5 py-0.5 rounded-md bg-[#221338] text-[#A78BFA] border border-[#3B1F60] hover:bg-[#6D28D9] hover:text-white transition-colors cursor-pointer font-semibold"
          >
            {cmd}
          </button>
        ))}
      ...
      <div className="p-4 sm:p-5 font-mono text-xs sm:text-sm max-h-72 overflow-y-auto space-y-3 bg-[#0B0711]/95 text-slate-100">
        {history.map((item, idx) => (
          <div key={idx} className="space-y-1">
            ...
            <div className="text-slate-100 font-normal leading-relaxed pl-3 border-l-2 border-[#6D28D9] bg-[#140D1F]/50 py-1 rounded-r-md">
        ...
        <form onSubmit={handleSubmit} className="flex items-center gap-2 pt-2">
          ...
          <input
            ...
            className="w-full bg-transparent text-white font-semibold focus:outline-none font-mono text-xs sm:text-sm placeholder:text-slate-500"
          />
          <button type="submit" className="text-slate-400 hover:text-[#A78BFA] p-1">
